  ...
  return (
    <div
      className={`rounded-lg border p-4 ${
        field.mismatch ? "border-red-400" : "border-grey-300"
      }`}
    >
      {/* Header: label + source badge or section ref */}
      <div className="flex items-center gap-2">
        ...
        <span className="ml-auto shrink-0">
          ...
          ) : field.page && field.page.includes("§") ? (
            <Badge className="text-xs !px-1.5 !py-0">
              {field.page.split(", ").slice(1).join(", ")}
            </Badge>
          ) : null}
        </span>
      </div>
...
export default function FormDataByPage() {
  ...
  // Filter fields per group when searching
  const filteredGroups = new Map<string, FormField[]>();
  let totalResults = 0;
  for (const [page, fields] of pageGroups) {
    const filtered = query ? fields.filter((f) => fieldMatches(f, query)) : fields;
    if (filtered.length > 0) {
      filteredGroups.set(page, filtered);
      if (query) totalResults += filtered.length;
    }
  }
  ...
  return (
    <>
      ...
      {Array.from(filteredGroups.entries()).map(([page, fields]) => (
        <Collapsible
          key={page}
          title={page}
